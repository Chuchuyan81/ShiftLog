...
function getShiftVenueName(shift) {
    const fallbackName = shift.is_workday ? 'Не указано' : 'Выходной';
...
function getShiftProductName(shiftProduct, fallbackName = 'Неизвестная позиция') {
    if (shiftProduct.venue_products && shiftProduct.venue_products.name) {
        return shiftProduct.venue_products.name;
    }
    
    const product = products.find(p => p.id === shiftProduct.product_id);
    ...
    switch (sortValue) {
        ...
        case 'venue':
            sortedShifts.sort((a, b) => {
                const venueA = getShiftVenueName(a);
                const venueB = getShiftVenueName(b);
...
async function renderShiftsList() {
    ...
    sortedShifts.forEach(shift => {
        ...
        // Получаем название заведения из JOIN'а или массива venues как fallback
        const venueName = getShiftVenueName(shift);
        ...
        if (shift.is_workday) {
            ...
            if (shift.shift_products) {
                shift.shift_products.forEach(sp => {
                    const productName = getShiftProductName(sp, 'Неизвестно');
