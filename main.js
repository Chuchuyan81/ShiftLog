...
async function handleEmailConfirmation() {
    console.log('📧 Проверяем подтверждение email...');
    ...
    try {
        ...
        console.log('📧 URL параметры:', { accessToken: !!accessToken, refreshToken: !!refreshToken, tokenType, type });
        
        if (accessToken && refreshToken && (type === 'signup' || type === 'recovery')) {
            console.log(`📧 Найдены токены для ${type === 'signup' ? 'подтверждения регистрации' : 'сброса пароля'}`);
            ...
            if (error) {
                ...
                showMessage('Ошибка', 'Не удалось подтвердить email. Попробуйте войти в систему.');
                return false;
            }
            ...
            if (type === 'signup') {
                // Показываем сообщение об успешном подтверждении регистрации
                showMessage('Успешно!', 'Email подтвержден. Добро пожаловать!');
                
                // Загружаем данные пользователя и показываем главное приложение
                await loadUserData();
                showMainApp();
            } else if (type === 'recovery') {
                // Показываем форму для смены пароля
                showMessage('Подтверждение сброса пароля', 'Теперь вы можете установить новый пароль.');
                // Здесь можно добавить форму смены пароля или перенаправить на соответствующий экран
                showAuthScreen();
            }
        ...
        console.error('❌ Ошибка при обработке подтверждения email:', error);
        showMessage('Ошибка', 'Произошла ошибка при подтверждении email.');
        return false;
    }
}
...
async function initializeApp() {
    ...
    console.log('🔍 Шаг 2: Проверяем подтверждение email...');
    
    // Проверяем подтверждение email из URL
    const emailConfirmed = await handleEmailConfirmation();
    if (emailConfirmed) {
        console.log('✅ Email подтвержден, пользователь авторизован');
        // Если email подтвержден, функция handleEmailConfirmation уже загрузила данные
        // и показала приложение, поэтому завершаем инициализацию
        isInitialized = true;
        isInitializing = false;
        return;
    }
    
    console.log('🔍 Шаг 3: Проверяем сессию...');
    
    try {
        // Проверяем текущую сессию
        console.log('🔍 Шаг 4: Получаем сессию из Supabase...');
        ...
        if (session) {
            ...
            console.log('🔍 Шаг 6: Запускаем проверку сессии...');
            startSessionCheck();
            console.log('✅ Проверка сессии запущена');
            
            // Загружаем данные пользователя с таймаутом
            console.log('🔍 Шаг 7: Начинаем загрузку данных пользователя...');
            ...
            try {
                console.log('🔍 Шаг 8: Вызываем loadUserData()...');
                await loadUserData();
                ...
                console.log('🔍 Шаг 9: Очищен таймаут загрузки');
            }
            
            // Скрываем загрузку и показываем основное приложение
            console.log('🔍 Шаг 10: Скрываем загрузку и показываем приложение...');
            hideLoading();
            showMainApp();
            ...
            console.log('🔍 Шаг 5alt: Показываем экран авторизации...');
        ...
        console.log('🔍 Шаг 11: Настраиваем обработчики событий...');
        setupEventListeners();
        console.log('✅ Обработчики событий настроены');
        
        console.log('🔍 Шаг 12: Настраиваем auth listener...');
        // Настраиваем auth listener ПОСЛЕ основной инициализации
        setupAuthStateListener();
        console.log('✅ Auth listener настроен');
        
        console.log('🔍 Шаг 13: Инициализируем PWA...');
        // Инициализируем PWA ПОСЛЕ основной инициализации
        initializePWA();
        console.log('✅ PWA инициализирован');
        
        console.log('🔍 Шаг 14: Отмечаем инициализацию как завершенную...');
...
function setupSettingsListeners() {
    ...
    window.testProductValues = function() {
        ...
        const firstInput = allInputs.length > 0 ? allInputs[0] : null;
        const testProductId = firstInput ? firstInput.getAttribute('data-product-id') : null;
        if (testProductId) {
...
async function loadProducts() {
    ...
    try {
        ...
        console.log('📋 Финальный результат products:', { count: allProducts ? allProducts.length : 0, error: lastError });
...
async function loadShifts() {
    ...
    try {
        ...
        console.log('📋 Финальный результат shifts:', { count: basicShifts ? basicShifts.length : 0, error: lastError });
    ...
    switch (sortValue) {
        ...
        case 'venue':
            sortedShifts.sort((a, b) => {
                ...
                const venueA = (venueAData && venueAData.name) || (a.is_workday ? 'Не указано' : 'Выходной');
                const venueB = (venueBData && venueBData.name) || (b.is_workday ? 'Не указано' : 'Выходной');
    ...
    sortedShifts.forEach(shift => {
        ...
        if (shift.products && shift.products.length > 0) {
            ...
            shift.products.forEach(sp => {
                ...
                const joinedProduct = sp.venue_products;
                const fallbackProduct = products.find(p => p.id === sp.product_id);
                const productName = (joinedProduct && joinedProduct.name) ||
...
window.diagnoseShifts = async function() {
    ...
    if (currentUser && currentUser.id) {
        ...
        try {
            ...
            console.log('  - Количество смен в БД:', directShifts ? directShifts.length : 0);
