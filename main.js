...
setTimeout(async () => {
    ...
    if (!currentUser && !isInitializing) {
        ...
        try {
            ...
            const activeUser = session && session.session ? session.session.user : null;
            if (activeUser) {
    ...
    switch (sortValue) {
        ...
        case 'venue':
            sortedShifts.sort((a, b) => {
                ...
                const venueA = foundVenueA && foundVenueA.name ? foundVenueA.name : (a.is_workday ? 'Не указано' : 'Выходной');
                const venueB = foundVenueB && foundVenueB.name ? foundVenueB.name : (b.is_workday ? 'Не указано' : 'Выходной');
...
function exportData() {
    ...
    reportsShifts.forEach(shift => {
        const venueName = shift.venues && shift.venues.name ? shift.venues.name : (shift.is_workday ? 'Не указано' : 'Выходной');
